WITH user_monthly_stats AS (
    -- 1) CTE: агрегируем действи€ по мес€цам
    SELECT 
        ua.user_id,
...
ranked_users AS (
    -- 2) CTE: ранг пользователей внутри каждого мес€ца (кто сделал больше действий)
    SELECT 
        *,
...
-- 3) ‘инальный запрос: скольз€щее среднее за 3 мес€ца (оконна€ функци€ с ROWS)
SELECT 
    ru.user_id,
    ru.action_month,
    ru.actions_count,
    ...
    --  огортный признак: мес€ц первого действи€
    DATE_TRUNC('month', ru.first_action_date) AS cohort_month
FROM ranked_users ru
WHERE ru.rank_monthly <= 10   -- только топ-10 активных пользователей в мес€ц
